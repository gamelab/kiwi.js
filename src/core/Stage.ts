	...
	export class Stage {
		...
		public set alpha( value: number ) {
			if ( this._game.deviceTargetOption === Kiwi.TARGET_BROWSER ) {
				this.container.style.opacity =
					String( Kiwi.Utils.GameMath.clamp( value, 1, 0 ) );
			}
		...
		**/
		...
		public getOffsetPoint(
			element,
			output: Kiwi.Geom.Point = new Kiwi.Geom.Point ): Kiwi.Geom.Point {
		...
		private _createCompositeCanvas() {
			...
			**/
			...
			if ( this.ctx ) {
				this._renderer =
					new Kiwi.Renderers.CanvasRenderer( this._game );
			} else if ( this.gl ) {
				this._renderer =
					new Kiwi.Renderers.GLRenderManager( this._game );
			}
		...
		public setRGBColor( r: number, g: number, b: number ): any {
			...
			**/
			...
			if ( this._game.deviceTargetOption === Kiwi.TARGET_COCOON ) {
				...
				// will be hard.
				Kiwi.Log.log(
					"Debug canvas not supported in cocoon, " +
					"creating canvas and context anyway",
					"#debug-canvas" );
			}
		...
		private _scaleContainer() {
			...
			**/
			...
			if ( this._game.deviceTargetOption == Kiwi.TARGET_BROWSER ) {
				...
				// To Fit or STRETCH
				if (
					this._scaleType == Kiwi.Stage.SCALE_STRETCH ||
					this._scaleType == Kiwi.Stage.SCALE_FIT ) {
				...
				if ( this._scaleType == Kiwi.Stage.SCALE_FIT ) {
					this.container.style.height =
						String(
							( clientWidth / this._width ) * this._height ) +
						"px";
				}
		...
		private _checkVisibility( event ) {
			...
			**/
			...
			if (
				...
				event.type === "mozvisibilitychange" ||
				event.type === "webkitvisibilitychange" ||
				event.type === "msvisibilitychange" ) {
